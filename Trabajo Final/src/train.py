...
from environment.State import s0, T, dt, sigma, k, A, kappa

if __name__ == "__main__":
    env = AvellanedaEnv(s0, T, dt, sigma, k, A, kappa)
    stop_callback = StopTrainingOnNoModelImprovement(
        max_no_improvement_evals=10,
        min_evals=500,
        verbose=1
    )
    eval_callback = EvalCallback(env, best_model_save_path='./logs/risk_aversion_0_5',
